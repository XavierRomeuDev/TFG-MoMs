...
<body>
    ...
    <div class="container">
        <div class="d-flex align-items-center justify-content-between mb-3 mt-4">
            ...
            <a href="{{ route('dashboard.news.create', ['lang' => app()->getLocale()]) }}"
        ...
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach ($news as $new)
                        <tr>
                            <td class="name">{{ utf8_encode($new->title) }}</td>
                            <td>{{ $new->id }}</td>
                            <td class="options">

                                <form action="{{ route('new.destroy', $new->id) }}" method="POST">
                                    <a class="btn btn-primary"
                                        href="{{ route('dashboard.news.edit', ['lang' => app()->getLocale(), 'new' => $new->id]) }}">Edit</a>
                                    @csrf
                                    @method('DELETE')
